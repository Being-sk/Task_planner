...
const Dashboard = () => {
  ...
  return (
    <div className="dashboard-container">
      ...
      <AnimatePresence>
        {isFocusMode && focusTask && (
          ...
          >
            <button 
                onClick={stopFocus}
                style={{ position: 'absolute', top: '24px', right: '24px', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-primary)' }}
            >
                <X size={32} />
            </button>
            ...
            <div style={{ textAlign: 'center', maxWidth: '800px', width: '100%' }}>
                ...
                <h2 className="deep-focus-title" style={{ fontSize: '4rem', fontWeight: '800', letterSpacing: '-0.05em', lineHeight: 1.1, marginBottom: '48px', wordBreak: 'break-word' }}>
                    {focusTask.text}
                </h2>
                
                <div className="deep-focus-timer" style={{ fontSize: '6rem', fontWeight: '300', fontFamily: 'monospace', marginBottom: '64px', color: 'var(--accent-black)' }}>
      ...
      <main style={{ padding: 0 }}>
        ...
        <header style={{ marginBottom: '48px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '32px', flexWrap: 'wrap', gap: '16px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
                <div style={{ background: 'var(--bg-secondary)', padding: '12px 20px', borderRadius: '14px', border: '1.5px solid var(--border-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', boxShadow: 'var(--shadow-sm)' }}>
        ...
        <section style={{ marginBottom: '48px' }}>
          <form onSubmit={handleAddTask} style={{ position: 'relative', filter: 'drop-shadow(0 10px 15px rgba(0,0,0,0.03))' }}>
            <input type="text" placeholder="Deploy a new task..." className="glass-input" style={{ padding: '24px 110px 24px 24px', fontSize: '1.1rem', borderRadius: '20px', borderWidth: '2px' }} value={newTask} onChange={(e) => setNewTask(e.target.value)} />
            <button type="submit" className="btn-primary" style={{ position: 'absolute', right: '8px', top: '8px', bottom: '8px', borderRadius: '14px', padding: '0 20px' }}>
              <Plus size={20} strokeWidth={2.5} /> <span className="queue-text">Queue</span>
            </button>
          </form>
        </section>
        ...
        <section>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            {loading ? (
              ...
              <div style={{ textAlign: 'center', padding: '80px 20px', background: 'var(--bg-tertiary)', borderRadius: '24px', border: '2px dashed var(--border-medium)', opacity: 0.8 }}>
                <div style={{ color: 'var(--text-tertiary)', marginBottom: '20px' }}><Calendar size={48} strokeWidth={1} /></div>
              ...
              <AnimatePresence mode="popLayout">
                {filteredTasks.map((task, index) => (
                  <motion.div key={task.id} layout initial={{ opacity: 0, scale: 0.98, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.98 }} transition={{ duration: 0.3, ease: [0.23, 1, 0.32, 1] }} className="glass-card task-card" style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '12px', background: 'var(--bg-secondary)', borderRadius: '20px', border: task.completed ? '1.5px solid transparent' : '1.5px solid var(--border-light)', opacity: task.completed ? 0.6 : 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                      <div onClick={() => toggleTask(task.id, task.completed)} style={{ cursor: 'pointer', color: task.completed ? 'var(--accent-black)' : 'var(--border-medium)', transition: 'all 0.3s ease', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        {task.completed ? (
                          <div style={{ width: '28px', height: '28px', background: 'var(--accent-black)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white' }}><Check size={16} strokeWidth={4} /></div>
                        ) : (
                          <div style={{ width: '28px', height: '28px', borderRadius: '50%', border: '2.5px solid currentColor', transition: 'all 0.2s ease' }} />
                        )}
                      </div>
                      ...
                      {editingTaskId === task.id ? (
                        ...
                        <>
                          <span onDoubleClick={() => handleEditTask(task)} style={{ flex: 1, fontSize: '1.1rem', fontWeight: '500', textDecoration: task.completed ? 'line-through' : 'none', color: task.completed ? 'var(--text-tertiary)' : 'var(--text-primary)', cursor: 'text', letterSpacing: '-0.02em', wordBreak: 'break-word' }}>{task.text}</span>
                          <div className="task-actions" style={{ display: 'flex', gap: '4px' }}>
                            <button onClick={() => startFocus(task)} title="Focus Mode" style={{ background: 'none', border: 'none', color: 'var(--text-tertiary)', cursor: 'pointer', padding: '6px' }}><Maximize2 size={16} /></button>
                            <button onClick={() => handleAtomize(task)} title="Atomize" style={{ background: 'none', border: 'none', color: 'var(--text-tertiary)', cursor: 'pointer', padding: '6px' }}><Zap size={16} /></button>
                            <button onClick={() => handleGetResources(task)} title="Resources" style={{ background: 'none', border: 'none', color: 'var(--text-tertiary)', cursor: 'pointer', padding: '6px' }}><Link size={16} /></button>
                            <button onClick={() => handleDeleteTask(task.id)} title="Delete" style={{ background: 'none', border: 'none', color: '#fca5a5', cursor: 'pointer', padding: '6px' }}><Trash2 size={16} /></button>
                          </div>
                        </>
                      )}
                    </div>
                    ...
                    {(task.subtasks?.length > 0 || task.resources?.length > 0) && (
                      <div style={{ padding: '12px 0 0 44px', borderTop: '1px solid var(--border-light)', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        {task.subtasks?.length > 0 && (
                          <div>
                            <div style={{ fontSize: '0.65rem', fontWeight: '700', textTransform: 'uppercase', color: 'var(--text-tertiary)', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}><Zap size={10} /> Sub-tasks</div>
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                              {task.subtasks.map((st, i) => (
                                <div key={i} style={{ padding: '4px 10px', background: 'var(--bg-tertiary)', borderRadius: '6px', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>{st}</div>
                              ))}
                            </div>
                          </div>
                        )}
                        {task.resources?.length > 0 && (
                          <div>
                            <div style={{ fontSize: '0.65rem', fontWeight: '700', textTransform: 'uppercase', color: 'var(--text-tertiary)', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}><Compass size={10} /> Resources</div>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                              {task.resources.map((res, i) => (
                                <a key={i} href={res.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: '0.85rem', color: 'var(--text-primary)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                  <ExternalLink size={12} /> <span style={{ textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }}>{res.title}</span>
                                </a>
                              ))}
                            </div>
                          </div>
                    ...
                    {processingTaskId === task.id && (
                      <div style={{ padding: '8px 44px' }}><div className="loader" style={{ width: '14px', height: '14px', borderWidth: '2px' }}></div></div>
                    )}
                  </motion.div>
                ))}
              </AnimatePresence>
      ...
      {/* --- SIDEBAR --- */}
      <aside className="sidebar" style={{ position: 'sticky', top: '120px', display: 'flex', flexDirection: 'column', gap: '32px' }}>
      ...
      <style>{`
        ...
        @media (max-width: 768px) {
            ...
            .queue-text { display: none; }
            .btn-primary span:not(.queue-text) { display: inline !important; }
        }
      `}</style>
    </div>
  );
